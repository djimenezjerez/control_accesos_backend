...
class Sensor {
  constructor(direccion = 'ffffffff', tamanoPaquete = 128, velocidad = 57600, cabecera = 'ef01') {
    this.tamanos = [];
  ...
  _armarInstruccion(com, param = '', tamRespuesta = '0c', respuesta = '09') {
    if(typeof tamRespuesta == 'number' && tamRespuesta < 48) {
      var tamanoRespuesta = this._int2hex(tamRespuesta, 2);
    }
    else {
      if(this._verificarPaqueteValido(tamRespuesta)) {
        var tamanoRespuesta = tamRespuesta;
      }
      else {
        logger.error('Tamaño de respuesta inválido');
        process.exit(1);
        return false;
      };
    };
    if(typeof respuesta == 'number' && respuesta < 48) {
      var byteRespuesta = this._int2hex(respuesta, 2);
    }
    else {
      if(this._verificarPaqueteValido(respuesta)) {
        var byteRespuesta = respuesta;
      }
      else {
    ...
    if(this.comandos.hasOwnProperty(com)) {
      ...
      return tamanoRespuesta + byteRespuesta + paquete;
    }
    else {
  ...
  handshake() {
    return this._armarInstruccion('handshake', '00');
  };
  ...
  setAdder(direccionNueva) {
    ...
    let paquete = this._armarInstruccion('setAdder', parametro);
  ...
  setSysPara(parametro, opcion) {
    ...
    if(valido) {
      return this._armarInstruccion('setSysPara', (this._int2hex(parametro, 2) + this._int2hex(opcion, 2)));
    }
    else {
      return false;
    };
  ...
  readSysPara(parametro = 9) {
    ...
    return this._armarInstruccion('readSysPara', '', 28, parametro);
  };

  templateNum() {
    logger.info('Leyendo numero total de plantillas en el sensor');
    return this._armarInstruccion('templateNum', '', 14, 11)
  };

  genImg() {
    logger.info('Coloque su dedo en el sensor');
    return this._armarInstruccion('genImg');
  };

  upImage() {
    logger.info('Enviando imagen a la PC ...');
    return this._armarInstruccion('upImage');
  };

  downImage() {
    logger.info('Enviando imagen al sensor ...');
    return this._armarInstruccion('downImage');
  };
  ...
  img2Tz(buffer = '01') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      return this._armarInstruccion('img2Tz', buffer)
    };
  };

  regModel() {
    logger.info('Combinar informacion de archivo de caracteres');
    return this._armarInstruccion('regModel');
  };
  ...
  upChar(buffer = '01') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      logger.info('Enviando archivo de caracteres a la PC ...');
      return this._armarInstruccion('upChar', buffer);
    };
  };
  ...
  downChar(buffer = '01') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      return this._armarInstruccion('downChar', buffer);
    };
  };
  ...
  store(id, buffer = '01') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      if(typeof id == 'number' && id > 1000) {
        ...
        return this._armarInstruccion('store', parametro);
      };
    };
  ...
  loadChar(id, buffer = '01') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      if(typeof id == 'number' && id > 1000) {
        ...
        let parametro = `${buffer}${id}`;
        return this._armarInstruccion('loadChar', parametro);
      };
      return this.handshake();
    };
  };
  ...
  deletChar(id, n = '0001') {
    ...
    if(typeof id == 'number' && id > 1000) {
      ...
      let parametro = `${id}${n}`;
      return this._armarInstruccion('deletChar', parametro);
    };
    return this.handshake();
  };
  ...
  empty() {
    ...
    return this._armarInstruccion('empty');
  };

  match() {
    logger.info('Comparacion de huellas entre los dos buffers');
    return this._armarInstruccion('match', '', 14, 11)
  };
  ...
  search(buffer = '01', inicio = '0000', fin = '03e8') {
    ...
    if(buffer != '01' && buffer != '02') {
      ...
      if(typeof fin == 'number' && fin >= 255) {
        ...
        return this._armarInstruccion('search', parametro, 16, 11);
      };
      return this.handshake();
    };
  };
