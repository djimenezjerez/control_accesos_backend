...
let sequelize = new Sequelize(cfg.baseDatos.bd, cfg.baseDatos.usuario, cfg.baseDatos.clave, {
  host: cfg.baseDatos.servidor,
  port: cfg.baseDatos.puerto,
  dialect: cfg.baseDatos.tipo,
...
.then(() => {
  logger.info(`Conectado a base de datos ${cfg.baseDatos.bd}`);
})
.catch((error) => {
  logger.error(`No se puede conectar con la base de datos ${cfg.baseDatos.bd}`);
  process.exit(1);
});
